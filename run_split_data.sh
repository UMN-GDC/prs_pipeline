...
#
# Accepts the same flags as src/run_split_plink_data.sh and forwards
# them after launching the container. See that script for full docs.
#
# Examples:
#   sbatch run_split_data.sh -1 /path/to/anc1 -2 /path/to/anc2 -r /path/to/prs_pipeline
...
Options: (see src/run_split_plink_data.sh for full details)
  -1 <path>    Target ancestry PLINK prefix (required)
  -2 <path>    Training ancestry PLINK prefix (required)
...
Examples:
  sbatch $0 -1 /path/to/AFR_study_sample -2 /path/to/EUR_study_sample -r /path/to/prs_pipeline
...
if [[ -z "$plink_file_anc1" || -z "$plink_file_anc2" || -z "$path_to_repo" ]]; then
    echo "ERROR: -1, -2, and -r are required"
    usage
fi
...
auto_bind() {
    local binds=""
    for path in "$plink_file_anc1" "$plink_file_anc2" "$path_to_repo"; do
        local dir
        dir=$(dirname "$path")
        local root
