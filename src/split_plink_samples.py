...
def main():
    parser = argparse.ArgumentParser(
        description="Split PLINK .bed files into GWAS/train/val/test subsets"
    )
    parser.add_argument("plink_prefix", help="Path to PLINK file prefix (e.g., /path/to/data/prefix)")
    parser.add_argument("--gwas", type=float, default=40.0, help="Percent of data for GWAS (default: 40)")
    parser.add_argument("--train", type=float, default=30.0, help="Percent of data for training (default: 30)")
    parser.add_argument("--val", type=float, default=10.0, help="Percent of data for validation (default: 10)")
    parser.add_argument("--test", type=float, default=20.0, help="Percent of data for testing (default: 20)")
    parser.add_argument("--no_plink", action="store_true", help="Only split samples, don't call PLINK")
    parser.add_argument("--seed", type=int, default=42, help="Random seed (default: 42)")
    args = parser.parse_args()

    # Validate split proportions
    total_pct = args.gwas + args.train + args.val + args.test
    ...
    n = len(df)
    n_gwas = int(n * args.gwas / 100)
    n_train = int(n * args.train / 100)
    n_val = int(n * args.val / 100)
    n_test = n - n_gwas - n_train - n_val

    # Split the dataset
    df_gwas = df.iloc[:n_gwas]
    df_train = df.iloc[n_gwas:n_gwas + n_train]
    df_val = df.iloc[n_gwas + n_train:n_gwas + n_train + n_val]
    df_test = df.iloc[n_gwas + n_train + n_val:]
